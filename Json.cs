...
public static partial class JsonSerializer {
	...
	private static void Serialize(StringBuilder sb, object? obj, LinkedElement<Type> linkedType, SerializationConfig config, int indentCount) {
		...
		try {
			if (config.SerializationPack.Any(s => s.Invoke(sb, obj, linkedType, config, indentCount, (o, lt, i) => Serialize(sb, o, lt, config, i))))
				return;
			if (!DefaultSerialization(sb, obj, linkedType, config, indentCount))
	...
	private static bool DefaultSerialization(StringBuilder sb, object obj, LinkedElement<Type> linkedType, SerializationConfig config, int indentCount) {
		...
		if (type.IsArray)
			return SerializeArray(sb, (Array)obj, linkedType, config, indentCount);
	...
	private static object? Deserialize(JsonReadBuffer buffer, LinkedElement<Type> linkedType, DeserializationConfig config) {
		...
		if (buffer.NextIsNull()) {
			if (type.IsClass || nullableValue != null)
				return null;
			throw new JsonReflectionException("This type isn't nullable");
		}
		if (nullableValue != null)
			return Deserialize(buffer, new(nullableValue, linkedType.Previous), config);
		...
		try {
			...
			if (type.IsInterface || type.IsAbstract)
				throw new JsonReflectionException("The type cannot be ethier an interface or an abstract class");
			object? result = null;
...
public sealed class JsonReadBuffer {
	...
	private void ReadLineBuffer() {
		...
		if (IsNEOB)
			return;
		if (!_nextLines.TryDequeue(out string? _buffer))
			throw new JsonSyntaxException($"Unexpected end of JSON input", this);
		LineIndex++;
		BufferIndex = 0;
