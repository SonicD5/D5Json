...
public static class JsonSystemPack {
	...
	public static readonly JsonSerialization CharSerialization = new() { 
		...
		Callback = (sb, obj, linkedType, config, _, _, _, ref hasSkiped) => { 
	...
	public static readonly Func<string?, JsonSerialization> DateTimeSerialization = format => new() {
		...
		Callback = (sb, obj, _, config, _, _, _, ref _) => { 
	...
	public static readonly JsonSerialization TypeSerialization = new() { 
		...
		JsonTypes = JsonTypes.String,
		Callback = (sb, obj, _, _, _, _, _, ref _) => sb.Append($"\"{StringType((Type)obj)}\"")
    };
	...
	public static readonly JsonSerialization StringKeyDictionarySerialization = new() {
		...
		Callback = (sb, obj, linkedType, config, indentCount, invoker, iDictType, ref hasCanceled) => {
            if (!linkedType.Value.GetGenericArguments()[0].HasJsonTypes(config.Pack, JsonTypes.String)) {
                hasCanceled = true;
                return;
            }
            ...
            LinkedElement<Type> linkedVType = new(iDictType.GetGenericArguments()[1], linkedType);
	...
	public static readonly JsonDeserialization StringKeyDictionaryDeserialization = new() {
		...
		JsonTypes = JsonTypes.Object,
		Callback = (ref buffer, linkedType, _, invoker, iDictType) => {
